...
exports.handler = async (ev) => {
  ...
  try {
    ...
    // Montar payload LabAnalysis
    const pl = {
      transformer_id, subscription_id: SI,
      data_coleta: data.coleta.data_coleta,
      numero_laudo: data.metadados.numero_relatorio,
      laboratorio: data.metadados.laboratorio,
      status_diagnostico: data.metadados.status_diagnostico,
      oil_type: data.equipamento.tipo_oleo || 'Mineral'
    };

    if (data.gases) {
      const g = data.gases;
      pl.h2   = g.hidrogenio_h2?.valor_atual_ppm;
      pl.ch4  = g.metano_ch4?.valor_atual_ppm;
      pl.c2h2 = g.acetileno_c2h2?.valor_atual_ppm;
      pl.c2h4 = g.etileno_c2h4?.valor_atual_ppm;
      pl.c2h6 = g.etano_c2h6?.valor_atual_ppm;
      pl.co   = g.monoxido_carbono_co?.valor_atual_ppm;
      pl.co2  = g.dioxido_carbono_co2?.valor_atual_ppm;
      pl.h2_anterior  = g.hidrogenio_h2?.valor_anterior_ppm;
      pl.co_anterior  = g.monoxido_carbono_co?.valor_anterior_ppm;
    ...
    if (data.fisico_quimico) {
      ...
      pl.teor_agua_ppm           = fq.teor_agua_ppm?.valor_atual;
      pl.rigidez_dieletrica_kv   = fq.rigidez_dieletrica_kv?.valor_atual;
      ...
      pl.cor                     = fq.cor?.valor_atual;
      pl.densidade               = fq.densidade_20_4_g_ml?.valor_atual;
      pl.aspecto_visual          = fq.aspecto_visual?.valor_atual;
      pl.furfural                = fq.furfural_mg_l?.valor_atual;
    }
    ...
    return { statusCode: 200, headers: cors, body: JSON.stringify({
      success: true, data, id: sv.b && sv.b.id, transformer_id, filename: filename || null
    })};
  } catch(e) {
